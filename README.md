...
- 我先用「我要一步一步做出畫面」的方式提問，從輸入框到按鈕再到影片顯示。
- 我要求用小學生能懂的話解釋，並避免專業術語。
- 我會指出我看不懂的地方，直接問「哪一行負責記住名字？」或「v-model、v-if、placeholder 是什麼？」，以及讓他把每一行的程式碼作講解，如果有不清楚地再分別詢問
- 我用「為什麼會出現／不出現」來確認邏輯，例如「為什麼我看不到影片？」、如何才能使影片顯示在畫面中，而不是只有連結等
...
- AI告訴我如何使用Youtube的API key來搜尋播放量最高的歌曲，ˇ以及一些注意的問題，例如要將API key存入隱藏檔中(env.local)
...
- 這個 `YOUTUBE_API_KEY` 是在 Google Cloud 申請的 YouTube Data API Key。
